...
void EnvironmentMaster::init() {
  ...
  robot_number_ = 0;
  robot_count_ = 0;
  std::ifstream f("/home/sneha/catkin_ws/src/warehouse/warehouse_manager/task900robot10.txt");
  if(f.is_open()){
    while(f){
      std::string line;
      std::getline(f, line);
...
{
  std::ofstream outfile;
  outfile.open("/home/sneha/catkin_ws/src/warehouse/warehouse_manager/task900robot10_report.txt", std::ios_base::app);
  if(robot_number >= 0 )
  {
    outfile << "\n" << "\n";
